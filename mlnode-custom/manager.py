import asyncio
from typing import Optional
from enum import Enum
from pydantic import BaseModel
from multiprocessing import Queue, Value
from ctypes import c_int

from pow.models.utils import Params
from pow.compute.controller import ParallelController
from common.logger import create_logger
from pow.service.sender import Sender
from pow.service.modal_worker import ModalWorker, warmup_modal
from pow.compute.utils import Phase
from common.manager import IManager


class PowState(Enum):
    IDLE = "IDLE"
    NO_CONTROLLER = "NOT_LOADED"
    LOADING = "LOADING"
    GENERATING = "GENERATING"
    VALIDATING = "VALIDATING"
    STOPPED = "STOPPED"
    MIXED = "MIXED"


class PowInitRequest(BaseModel):
    node_id: int = -1
    node_count: int = -1
    block_hash: str
    block_height: int
    public_key: str
    batch_size: int
    r_target: float
    fraud_threshold: float
    poc_stage_start_block_height: int = 0  # Required by API for batch submission
    params: Params = Params()


class PowInitRequestUrl(PowInitRequest):
    url: str


logger = create_logger(__name__)


class PowManager(IManager):
    def __init__(self):
        super().__init__()
        self.pow_controller: Optional[ParallelController] = None
        self.pow_sender: Optional[Sender] = None
        self.init_request: Optional[PowInitRequest] = None
        self.modal_worker: Optional[ModalWorker] = None
        self.modal_phase: Optional[Value] = None
        self.generated_batch_queue = None
        self._using_modal = False

    def switch_to_pow(self, init_request: PowInitRequest):
        if self.pow_controller is not None:
            logger.info("Stopping PoW controller")
            self.stop()
        self.init(init_request)
        self.start()
    
    async def switch_to_pow_async(self, init_request: PowInitRequest):
        return await asyncio.to_thread(self.switch_to_pow, init_request)

    def init(self, init_request: PowInitRequest):
        self.init_request = init_request
        # Use poc_stage_start_block_height if provided, otherwise fall back to block_height
        poc_block = init_request.poc_stage_start_block_height if init_request.poc_stage_start_block_height > 0 else init_request.block_height
        
        self.pow_controller = ParallelController(
            params=init_request.params,
            block_hash=init_request.block_hash,
            block_height=init_request.block_height,
            public_key=init_request.public_key,
            node_id=init_request.node_id,
            node_count=init_request.node_count,
            batch_size=init_request.batch_size,
            r_target=init_request.r_target,
            devices=None,
        )
        self.pow_sender = Sender(
            url=init_request.url,
            generation_queue=self.pow_controller.generated_batch_queue,
            validation_queue=self.pow_controller.validated_batch_queue,
            phase=self.pow_controller.phase,
            r_target=self.pow_controller.r_target,
            fraud_threshold=init_request.fraud_threshold,
            block_height=poc_block,
            node_id=init_request.node_id,
        )

    def _start(self):
        if self.pow_controller is None:
            raise Exception("PoW not initialized")
        if self.pow_controller.is_running():
            raise Exception("PoW is already running")
        logger.info(f"Starting controller with params: {self.init_request}")
        self.pow_controller.start()
        self.pow_sender.start()

    def get_pow_status(self) -> dict:
        if self._using_modal and self.modal_worker is not None and self.modal_worker.is_alive():
            return {
                "status": PowState.GENERATING.value,
                "is_model_initialized": True,
                "mode": "modal"
            }
        
        if self.pow_controller is None:
            return {"status": PowState.NO_CONTROLLER.value}
        
        phase = self.phase_to_state(self.pow_controller.phase.value)
        loading = not self.pow_controller.is_model_initialized()
        if loading and phase == PowState.IDLE:
            phase = PowState.LOADING
        return {"status": phase.value, "is_model_initialized": not loading}

    def _stop(self):
        self._using_modal = False
        
        if self.modal_worker is not None:
            logger.info("Stopping Modal worker")
            self.modal_worker.stop()
            self.modal_worker.join(timeout=5)
            if self.modal_worker.is_alive():
                self.modal_worker.terminate()
            self.modal_worker = None
        
        if self.pow_controller is not None:
            self.pow_controller.stop()
        
        if self.pow_sender is not None:
            self.pow_sender.stop()
            self.pow_sender.join(timeout=5)
            if self.pow_sender.is_alive():
                logger.warning("Sender did not stop, terminating")
                self.pow_sender.terminate()

        self.pow_controller = None
        self.pow_sender = None
        self.init_request = None

    @staticmethod
    def phase_to_state(phase: Phase) -> PowState:
        if phase == Phase.IDLE:
            return PowState.IDLE
        elif phase == Phase.GENERATE:
            return PowState.GENERATING
        elif phase == Phase.VALIDATE:
            return PowState.VALIDATING
        return PowState.IDLE

    def is_running(self) -> bool:
        if self._using_modal and self.modal_worker is not None and self.modal_worker.is_alive():
            return True
        return self.pow_controller is not None and self.pow_controller.is_running()

    def _is_healthy(self) -> bool:
        if self._using_modal and self.modal_worker is not None:
            return self.modal_worker.is_alive()
        return self.pow_controller is not None and self.pow_controller.is_alive()

    def start_modal_generation(self, init_request: PowInitRequest):
        """Start POW generation using Modal serverless GPU."""
        warmup_modal()
        self.stop()
        
        self._using_modal = True
        self.init_request = init_request
        self.generated_batch_queue = Queue()
        self.modal_phase = Value(c_int, Phase.GENERATE)
        
        # Use poc_stage_start_block_height if provided, otherwise fall back to block_height
        poc_block = init_request.poc_stage_start_block_height if init_request.poc_stage_start_block_height > 0 else init_request.block_height
        
        self.modal_worker = ModalWorker(
            phase=self.modal_phase,
            generated_batch_queue=self.generated_batch_queue,
            block_hash=init_request.block_hash,
            block_height=init_request.block_height,
            public_key=init_request.public_key,
            r_target=init_request.r_target,
            node_id=init_request.node_id,
            batch_size=init_request.batch_size,
            batches_per_call=20,
        )
        
        self.pow_sender = Sender(
            url=init_request.url,
            generation_queue=self.generated_batch_queue,
            validation_queue=Queue(),
            phase=self.modal_phase,
            r_target=init_request.r_target,
            fraud_threshold=init_request.fraud_threshold,
            block_height=poc_block,
            node_id=init_request.node_id,
        )
        
        logger.info(f"Starting Modal POW generation with poc_stage_start_block_height={poc_block}")
        self.modal_worker.start()
        self.pow_sender.start()

    async def start_modal_generation_async(self, init_request: PowInitRequest):
        return await asyncio.to_thread(self.start_modal_generation, init_request)
    
    def stop_modal(self):
        if self.modal_phase is not None:
            self.modal_phase.value = Phase.IDLE
        warmup_modal()
        self.stop()
